...
<template>
	...
	<view class="t-login">
		<view class="reset-title">重置密码</view>
...
export default {
	...
	methods: {
		...
		async login() {
			...
			const { data } = await request.httpRequest(opts,{email: this.form.email,passwd:this.form.passwd,code: this.form.emailCode})
		...
		async getCode() {
			if (this.$refs.uCode.canGetCode) {
				...
				// 	}
				// });
                const { data } = await request.httpRequest(opts,{email: this.form.email,type:'rest'})
                console.log(data,'data')
                setTimeout(() => {
